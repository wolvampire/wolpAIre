...
class OracleDecider(Decider):
    ...
    def best_troop_orders(self, faction, ally_tiles, enemy_tiles, human_tiles, layer=1, alpha=-float('inf'),
                          beta=float('inf'), return_troop_orders=True, \
    ...
    def check_conflict_and_compute_battle(self, temporary_new_ally_tiles, enemy_tiles, human_tiles,
                                          n_allies_previous_turn, turn, conflict_occured):
        new_enemy_tiles = enemy_tiles[:]
        new_human_tiles = human_tiles[:]
            ...
            for enemy in enemy_tiles:
                if ally.x == enemy.x and ally.y == enemy.y:
                    ...
                    if ally.nb >= enemy.nb * 1.5:
                        ...
                        new_enemy_tiles.append(BoardTile(enemy.x, enemy.y, nb=enemy.nb,
                                                         faction=Faction.WERE if turn == Faction.VAMP else Faction.VAMP))
    ...
    def heuristic(self, ally_tiles, enemy_tiles, human_tiles, faction, previous_ally_tiles, previous_enemy_tiles):
        ...
        score += turn_differential_coef * (
                sum([a.nb for a in ally_tiles]) - sum([a.nb for a in previous_ally_tiles]))
        score -= turn_differential_coef * (
                sum([e.nb for e in enemy_tiles]) - sum([e.nb for e in previous_enemy_tiles]))
